...
var setPagination = function (state, action) {
    ...
    return __assign(__assign({}, state), { page: page, perPage: perPage, total: total, lastPage: lastPage, from: from, to: to, hasPrevPage: page > 1, hasNextPage: !!lastPage && page < lastPage });
};
var reducer = function (state, action) {
    ...
    switch (action.type) {
        ...
        case 'set-data': {
            var selectableItemIds = (_a = action.payload.selectableItemIds) !== null && _a !== void 0 ? _a : action.payload.data.map(function (x) { return x.id; });
            var selected = (_b = action.payload.selected) !== null && _b !== void 0 ? _b : [];
            return __assign(__assign({}, state), { data: action.payload.data, selectableItemIds: selectableItemIds, selected: (_c = action.payload.selected) !== null && _c !== void 0 ? _c : [], isAllSelected: !!selectableItemIds.length && selected.length == selectableItemIds.length });
        }
        case 'set-selected':
            return __assign(__assign({}, state), { selected: action.payload.ids, isAllSelected: action.payload.ids.length === state.selectableItemIds.length });
        case 'toggle-selected': {
            ...
            return __assign(__assign({}, state), { selected: temp, isAllSelected: temp.length === state.selectableItemIds.length });
        }
