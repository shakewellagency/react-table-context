...
var setPagination = function (state, action) {
    ...
    return __assign(__assign({}, state), { page: page, perPage: perPage, total: total, lastPage: lastPage, from: from, to: to, hasPrevPage: page > 1, hasNextPage: !!state.lastPage && page < state.lastPage });
};
var reducer = function (state, action) {
    ...
    switch (action.type) {
        ...
        case 'set-selected':
            return __assign(__assign({}, state), { selected: action.payload.ids, isAllSelected: action.payload.ids.length === state.data.length });
